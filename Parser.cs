...
public class Parser {
	...
	public const int lbrackrbrack_Sym = 40;
	public const int int_Sym = 41;
	public const int bool_Sym = 42;
	public const int char_Sym = 43;
	public const int barbar_Sym = 44;
	public const int star_Sym = 45;
	public const int slash_Sym = 46;
	public const int andand_Sym = 47;
	public const int percent_Sym = 48;
	public const int equalequal_Sym = 49;
	public const int bangequal_Sym = 50;
	public const int less_Sym = 51;
	public const int lessequal_Sym = 52;
	public const int greater_Sym = 53;
	public const int greaterequal_Sym = 54;
	...
	static void Statement() {
		if (la.kind == lbrace_Sym) {
			...
			ForLoop();
		} else if (la.kind == set_Sym || la.kind == setlbrackrbrack_Sym) {
			Set();
		} else SynErr(56);
	}
	...
	static void Set() {
		...
		} else if (la.kind == setlbrackrbrack_Sym) {
			Get();
			Expect(identifier_Sym);
			Expect(equal_Sym);
			Expect(new_Sym);
			Expect(setlbrack_Sym);
			Expect(number_Sym);
			Expect(rbrack_Sym);
		} else SynErr(59);
		Expect(semicolon_Sym);
	...
	static bool[,] set = {
		...
		{x,T,x,x, x,x,x,x, T,x,T,T, x,x,x,x, x,x,x,T, T,T,x,T, T,x,T,T, T,T,T,x, x,T,T,T, T,x,x,x, x,T,T,T, x,x,x,x, x,x,x,x, x,x,x,x, x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,T,T,T, T,T,T,x, x},
		{x,T,T,x, T,x,T,x, x,x,x,x, x,x,T,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,x, x,T,T,T, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,T,T,T, T,x,x,x, x,x,x,x, x},
		{x,x,T,x, T,x,x,x, x,x,x,x, x,x,T,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x}
...
public class Errors {
	...
	public static void SynErr (int line, int col, int n) {
		...
		switch (n) {
			...
			case 57: s = "invalid Designator2"; break;
			case 58: s = "invalid Designator2"; break;
			case 59: s = "invalid Set"; break;
